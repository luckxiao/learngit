...
router.post('/login', (req, res) => {
    ...
    for (var key in params){
        ...
        //console.log(JSON.parse(key).name);
            // 输出地是 select * from user
    name = JSON.parse(key).name;
    password = JSON.parse(key).password;
    if (name) {
        sql_name += " where username ='"+name +"'";
        console.log(sql_name);
    }   
    conn.query(sql_name, name, function(err, result) {
        if (err) {
            console.log(err);
        }
        // console.log(result);
        if (result[0] === undefined) {
            ...
            if(resultArray.password === password) {
                jsonWrite(res, result);
